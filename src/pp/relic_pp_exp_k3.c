...
void pp_exp_k3(fp12_t c, fp12_t a) {
	...
	RLC_TRY {
		...
		if (bn_sign(x) == RLC_NEG && b[0] == 0 && b[1] == -1) {
			fp12_exp_cyc_sps(t0, c, b+2, l-2, RLC_POS);
			fp12_exp_cyc_sps(t0, t0, b+2, l-2, RLC_POS);
		} else {
			fp12_exp_cyc_sps(t1, c, b, l, bn_sign(x));
			fp12_inv_cyc(t0, c);
			fp12_mul(t1, t1, t0);
			fp12_exp_cyc_sps(t0, t1, b, l, bn_sign(x));
			fp12_inv_cyc(t1, t1);
			fp12_mul(t0, t0, t1);
		}
		...
		/* t0 = t1^(x^2 + p^2). */
		fp12_exp_cyc_sps(t0, t1, b, l, RLC_POS);
		fp12_exp_cyc_sps(t0, t0, b, l, RLC_POS);
		fp12_frb(t1, t1, 2);
		fp12_mul(t0, t0, t1);

		/* t1 = t0^(x^4 + p^4 - 1). */
		fp12_exp_cyc_sps(t1, t0, b, l, RLC_POS);
		fp12_exp_cyc_sps(t1, t1, b, l, RLC_POS);
		fp12_exp_cyc_sps(t1, t1, b, l, RLC_POS);
		fp12_exp_cyc_sps(t1, t1, b, l, RLC_POS);
		fp12_inv_cyc(t2, t0);
		fp12_frb(t0, t0, 4);
