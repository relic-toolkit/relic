...
/**
 * Size of the per-round hash buffer, sized for the parallel argument:
 * statement digest, L, R, L~, R~, prior challenge.  The plain argument uses
 * a prefix of it and hashes only the bytes it wrote.
 */
#define CP_IPA_BUF		(RLC_MD_LEN + 5 * RLC_FP_BYTES + 4)

/**
 * Digests the statement into a fixed-size prefix for every challenge.
 *
 * With par = 0 this covers pp = (n, g, u), the commitment M and the claimed
 * inner product c, as Fig. 4 and Fig. 5 require: binding P is equivalent to
 * binding (M, c) since P = M * u^(x_M * c).  With par = 1 it additionally
 * covers the second generator vector g~ and commitment M~, because Appendix E
 * runs one challenge chain across both statements.
 *
 * A mode byte follows the domain separator so the two cases cannot collide
 * even where the remaining inputs would coincide.  Fixed-width encodings
 * throughout, so the concatenation admits no reparsing as a different (n, g).
 *
 * @param[out] dst			- the resulting digest, RLC_MD_LEN bytes.
 * @param[in] g				- the generator vector.
 * @param[in] gt			- the second generator vector, ignored if !par.
 * @param[in] u				- the auxiliary generator.
 * @param[in] m				- the commitment M = g^a.
 * @param[in] mt			- the commitment M~ = g~^a, ignored if !par.
 * @param[in] c				- the claimed inner product, reduced mod the order.
 * @param[in] n				- the number of generators.
 * @param[in] par			- 1 for the parallel argument, 0 otherwise.
 */
static void cp_ipa_bind(uint8_t *dst, const ec_t *g, const ec_t *gt,
		const ec_t u, const ec_t m, const ec_t mt, const bn_t c, size_t n,
		int par) {
	const size_t unit = RLC_FP_BYTES + 1;
	size_t pts = (par ? 2 * n + 3 : n + 2);
	size_t len = sizeof(CP_IPA_DST) - 1 + 1 + 8 + pts * unit + RLC_FP_BYTES;
...
/**
 * Digests the statement of the parallel argument: pp = (n, g, g~, u), both
 * commitments and the claimed inner product.  Appendix E runs one challenge
 * chain across both statements, so both must enter the digest.
 */

/**
 * Derives round challenge i of the parallel argument.  One chain covers both
 * statements, which is what distinguishes ParInPrd~ from a mere parallel
 * repetition of two independent arguments.
 *
 * @return 0 if the challenge is zero, in which case the caller must fail.
 */

/**
 * Derives the coefficient merging the two verification equations into one
 * multi-exponentiation.  Seeded with the final chain value, so it is fixed
 * only once every L, R, L~, R~ is.
 */
static void cp_ipa_merge(bn_t rho, const uint8_t *dst, const bn_t last,
		const bn_t r) {
	uint8_t buf[RLC_MD_LEN + RLC_FP_BYTES + 1], hash[RLC_MD_LEN];

	memcpy(buf, dst, RLC_MD_LEN);
	bn_write_bin(buf + RLC_MD_LEN, RLC_FP_BYTES, last);
	buf[RLC_MD_LEN + RLC_FP_BYTES] = 0x02;
	md_map(hash, buf, sizeof(buf));
	bn_read_bin(rho, hash, RLC_MD_LEN);
	bn_mod(rho, rho, r);
	if (bn_is_zero(rho)) {
		bn_set_dig(rho, 1);
	}
...
/**
 * Derives x_M, the challenge shifting u in Protocol 1 of [13].  Labelled
 ...
 *
 * With par = 1 the round elements of both statements enter the same hash,
 * which is what distinguishes ParInPrd~ from a parallel repetition of two
 * independent arguments.  Only the bytes actually written are hashed, so the
 * two modes consume different-length inputs.
 *
 * @return 0 if the challenge is zero, in which case the caller must fail.
 */
static int cp_ipa_next(bn_t x, uint8_t *buf, const ec_t l, const ec_t rr,
		const ec_t lt, const ec_t rt, const bn_t r, int par) {
	uint8_t hash[RLC_MD_LEN];
	const size_t unit = RLC_FP_BYTES + 1;
	size_t o = RLC_MD_LEN;

	ec_write_bin(buf + o, unit, l, 1);
	o += unit;
	ec_write_bin(buf + o, unit, rr, 1);
	o += unit;
	if (par) {
		ec_write_bin(buf + o, unit, lt, 1);
		o += unit;
		ec_write_bin(buf + o, unit, rt, 1);
		o += unit;
	}
	bn_write_bin(buf + o, RLC_FP_BYTES, x);
	o += RLC_FP_BYTES;
...
int cp_ipa_prv(bn_t y, ec_t p, ec_t *ls, ec_t *rs, const ec_t *g, const bn_t *a,
	...
	RLC_TRY {
		...
		/* Protocol 1 of [13]: draw x_M once the statement is fixed, then run
		 * the argument against the shifted generator u' = u^(x_M). */
		cp_ipa_bind(dst, g, NULL, u, p, NULL, cc, n, 0);
		...
		for (size_t i = 0; i < k; i++) {
			...
			if (!cp_ipa_next(x, buf, ls[i], rs[i], NULL, NULL, r, 0)) {
				RLC_THROW(ERR_NO_VALID);
			}
...
int cp_ipa_ver(const bn_t y, const ec_t p, const bn_t c, const ec_t *ls,
	...
	RLC_TRY {
		 ...
		 * same three helpers.
		 *-----------------------------------------------------------------*/
		cp_ipa_bind(dst, g, NULL, u, p, NULL, w, n, 0);
		...
		for (size_t i = 0; result && i < k; i++) {
			if (!cp_ipa_next(t, buf, ls[i], rs[i], NULL, NULL, r, 0)) {
				result = 0;
				break;
			}
			bn_copy(ch[i], t);
...
int cp_ipa_prv_par(bn_t y, ec_t p, ec_t q, ec_t *ls, ec_t *rs, ec_t *lt,
	...
	if (n == 0 || b == NULL || c == NULL || h == NULL || ht == NULL) {
		RLC_FREE(b);
		RLC_FREE(c);
		RLC_FREE(h);
		RLC_FREE(ht);
	...
	RLC_TRY {
		...
		bn_zero(cc);
		for (size_t i = 0; i < (1 << k); i++) {
			bn_null(b[i]);  bn_new(b[i]);
			bn_null(c[i]);  bn_new(c[i]);
			ec_null(h[i]);  ec_new(h[i]);
			ec_null(ht[i]); ec_new(ht[i]);
			bn_set_dig(b[i], 1);
			if (i < n) {
				ec_copy(h[i], g[i]);
				ec_copy(ht[i], gt[i]);
		...
		/* Both commitments share the single witness a. */
		ec_mul_sim_lot(p, g, a, n);
		ec_norm(p, p);
		ec_mul_sim_lot(q, gt, a, n);
		ec_norm(q, q);

		cp_ipa_bind(dst, g, gt, u, p, q, cc, n, 1);
		if (!cp_ipa_shift(x, dst, r)) {
			RLC_THROW(ERR_NO_VALID);
		}
		ec_mul(uu, u, x);
		...
		for (size_t i = 0; i < k; i++) {
			...
			ec_add(rs[i], rs[i], s);
			ec_norm(rs[i], rs[i]);
			ec_mul_sim_lot(rt[i], ht, c + m, m);
			ec_add(rt[i], rt[i], s);
			ec_norm(rt[i], rt[i]);

			if (!cp_ipa_next(x, buf, ls[i], rs[i], lt[i], rt[i], r, 1)) {
				RLC_THROW(ERR_NO_VALID);
			}
			...
			for (size_t j = 0; j < m; j++) {
				ec_mul_sim(h[j], h[j], t, h[m + j], x);
				ec_mul_sim(ht[j], ht[j], t, ht[m + j], x);
				bn_mul(c[j], c[j], x);
				bn_mul(c[m + j], c[m + j], t);
				bn_add(c[j], c[j], c[m + j]);
				bn_mod(c[j], c[j], r);
		...
		/* One shared final scalar, per Appendix E: this is the whole size
		 * saving over two independent arguments. */
		bn_copy(y, c[0]);
	} RLC_CATCH_ANY {
		result = RLC_ERR;
	} RLC_FINALLY {
...
int cp_ipa_ver_par(const bn_t y, const ec_t p, const ec_t q, const bn_t c,
	...
	int result = 1;
	size_t k, big, len, o;
	bn_t t, r, w, v, xm, rho, *ch, *iv, *e;
	ec_t z, *h;

	k = cp_ipa_rounds(n);
	big = ((size_t)1) << k;
	/* g, g~, four round vectors, u, and both commitments. */
	len = 2 * n + 4 * k + 3;

	ch = RLC_ALLOCA(bn_t, k + 1);
	iv = RLC_ALLOCA(bn_t, k + 1);
	/* big slots for the s-vector, then 2n scaled copies and the tail;
	 * big >= n, so 2 * big + 4k + 3 covers both layouts. */
	e  = RLC_ALLOCA(bn_t, 2 * big + 4 * k + 3);
	h  = RLC_ALLOCA(ec_t, len);
	...
	RLC_TRY {
		...
		for (size_t i = 0; i < 2 * big + 4 * k + 3; i++) {
			bn_null(e[i]); bn_new(e[i]);
		}
		for (size_t i = 0; i < len; i++) {
			ec_null(h[i]); ec_new(h[i]);
		...
		cp_ipa_bind(dst, g, gt, u, p, q, w, n, 1);
		if (!cp_ipa_shift(xm, dst, r)) {
			result = 0;
		}
		...
		bn_zero(t);
		for (size_t i = 0; result && i < k; i++) {
			if (!cp_ipa_next(t, buf, ls[i], rs[i], lt[i], rt[i], r, 1)) {
				result = 0;
				break;
			}
			bn_copy(ch[i], t);
		...
		if (result) {
			/* t still holds the final chain value, so rho is fixed only
			 * after every round element has been read. */
			cp_ipa_merge(rho, dst, t, r);

			if (k > 0) {
				bn_mod_inv_sim(iv, ch, r, k);
			}

			/* Shared s-vector: both statements fold under one chain. */
			bn_set_dig(e[0], 1);
			for (size_t i = 0; i < k; i++) {
				bn_mul(e[0], e[0], iv[i]);
				bn_mod(e[0], e[0], r);
			...
			ec_mul_sim_lot(z, h, e, len);
			result = ec_is_infty(z);
		}
	} RLC_CATCH_ANY {
		result = 0;
	} RLC_FINALLY {
