...
/**
 * Length in bytes of the salt randomizing the hash-to-group evaluation of the
 * time-deniable signature scheme.
 */
#define RLC_TDS_SALT	16

/**
 * Public exponent of the timestamp chain and of the delayed evaluation of the
 * time-deniable signature scheme. It must be odd, greater than 1, and admit a
 * short addition chain, since it is applied once per sequential step.
 */
#define RLC_TDS_EXPO	3
...
/**
 * Represents a time-deniable signature key pair. The group is the group of
 * units modulo the modulus, quotiented by the subgroup generated by minus one.
 */
typedef struct {
	/** The modulus defining the group of unknown order. */
	bn_t n;
	/** The public exponent. */
	bn_t e;
	/** The onset value of the timestamp chain. */
	bn_t z;
	/** The group order, known only to the signer. */
	bn_t ord;
	/** The inverse of the public exponent modulo the group order. */
	bn_t d;
	/** The factorization of the modulus, for exponentiation by the CRT. */
	crt_t crt;
	/** The delay parameter. */
	size_t t;
	/** The upper bound on the timestamp space. */
	size_t l;
} _tds_st;

/**
 * Pointer to a time-deniable signature key pair.
 */
#if ALLOC == AUTO
typedef _tds_st tds_t[1];
#else
typedef _tds_st *tds_t;
#endif

/**
 * Represents a time-deniable signature.
 */
typedef struct {
	/** The timestamp representative. */
	bn_t z;
	/** The delayed output. */
	bn_t y;
	/** The proof of exponentiation for the timestamp chain. */
	bn_t ts;
	/** The proof of exponentiation for the delayed output. */
	bn_t del;
	/** The salt. */
	uint8_t s[RLC_TDS_SALT];
} _tds_sig_st;

/**
 * Pointer to a time-deniable signature.
 */
#if ALLOC == AUTO
typedef _tds_sig_st tds_sig_t[1];
#else
typedef _tds_sig_st *tds_sig_t;
#endif
...
/**
 * Initializes a time-deniable signature key pair with a null value.
 *
 * @param[out] A			- the key pair to initialize.
 */
#define tds_null(A)			RLC_NULL(A)

/**
 * Calls a function to allocate and initialize a time-deniable signature key
 * pair.
 *
 * @param[out] A			- the new key pair.
 */
#if ALLOC == DYNAMIC
#define tds_new(A)															\
	A = (tds_t)calloc(1, sizeof(_tds_st));									\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	bn_null((A)->n);														\
...
/**
 * Calls a function to clean and free a time-deniable signature key pair.
 *
 * @param[out] A			- the key pair to clean and free.
 */
#if ALLOC == DYNAMIC
...
#elif ALLOC == AUTO
#define tds_free(A)			/* empty */
#endif

/**
 * Initializes a time-deniable signature with a null value.
 *
 * @param[out] A			- the signature to initialize.
 */
#define tds_sig_null(A)		RLC_NULL(A)

/**
 * Calls a function to allocate and initialize a time-deniable signature.
 *
 * @param[out] A			- the new signature.
 */
#if ALLOC == DYNAMIC
#define tds_sig_new(A)														\
	A = (tds_sig_t)calloc(1, sizeof(_tds_sig_st));							\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	bn_null((A)->z);														\
...
/**
 * Calls a function to clean and free a time-deniable signature.
 *
 * @param[out] A			- the signature to clean and free.
 */
#if ALLOC == DYNAMIC
#define tds_sig_free(A)														\
	if (A != NULL) {														\
		bn_free((A)->z);													\
		bn_free((A)->y);													\
		bn_free((A)->ts);													\
...
/**
 * Generates a key pair for the time-deniable signature scheme. The delay
 * parameter is not evaluated during key generation.
 *
 * @param[out] pub			- the public key.
 * @param[out] prv			- the private key.
 * @param[in] bits			- the bit length of the modulus.
 * @param[in] delay			- the delay parameter.
 * @param[in] bound			- the upper bound on the timestamp space.
 * @return RLC_OK if no errors occurred, RLC_ERR otherwise.
 */
int cp_tds_gen(tds_t pub, tds_t prv, size_t bits, size_t delay, size_t bound);

/**
 * Signs a message with a timestamp using the hidden group order. Signing is
 * stateless and runs in time polynomial in the security parameter and in the
 * logarithms of the timestamp and of the delay parameter.
 *
 * @param[out] sig			- the resulting signature.
 * @param[in] salt			- the salt to use, or NULL to sample a fresh one.
 * @param[in] msg			- the message to sign.
...
/**
 * Verifies a time-deniable signature. Verification evaluates the hash-to-group
 * function once and checks two proofs of exponentiation.
 *
 * @param[in] sig			- the signature to verify.
 * @param[in] msg			- the signed message.
 * @param[in] len			- the length of the message in bytes.
 * @param[in] stamp			- the timestamp.
 * @param[in] pub			- the public key.
 * @return a boolean value indicating whether the signature is valid.
 */
int cp_tds_ver(const tds_sig_t sig, const uint8_t *msg, size_t len,
		size_t stamp, const tds_t pub);

/**
 * Computes a signature on a new message and timestamp from a valid signature,
 * without the private key, by carrying out the prescribed sequential work. The
 * target timestamp must not be more recent than the source timestamp.
 *
 * The witness for the delayed output is assembled from powers retained during
 * the sequential phase, so that the non-sequential overhead is sublinear in
 * the delay parameter. Short delays, and delays too long for the retained
 * powers to fit the bounds of the module, fall back to an assembly that runs
 * in constant memory instead.
 *
 * @param[out] alt			- the resulting signature.
 * @param[in] salt			- the salt to use, or NULL to sample a fresh one.
 * @param[in] msg			- the target message.
 * @param[in] len			- the length of the target message in bytes.
 * @param[in] stamp			- the target timestamp.
 * @param[in] sig			- the source signature.
 * @param[in] src			- the source message.
 * @param[in] src_len		- the length of the source message in bytes.
 * @param[in] src_stamp		- the source timestamp.
 * @param[in] pub			- the public key.
 * @return RLC_OK if no errors occurred, RLC_ERR otherwise.
 */
...
/**
 * Writes a time-deniable signature to a byte vector. The signature consists of
 * four group elements and one salt.
 *
 * @param[out] bin			- the resulting byte vector.
 * @param[in,out] len		- the buffer capacity on input, the length written on
 * 							output.
 * @param[in] sig			- the signature to write.
 * @param[in] pub			- the public key.
 * @return RLC_OK if no errors occurred, RLC_ERR otherwise.
 */
...
/**
 * Reads a time-deniable signature from a byte vector.
 *
 * @param[out] sig			- the resulting signature.
 * @param[in] bin			- the byte vector.
 * @param[in] len			- the length of the byte vector.
 * @param[in] pub			- the public key.
 * @return RLC_OK if no errors occurred, RLC_ERR otherwise.
 */
