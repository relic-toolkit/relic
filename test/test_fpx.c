...
static int cyclotomic48(void) {
	...
	TRY {
		...
		TEST_BEGIN("sparse cyclotomic exponentiation is correct") {
			...
			fp48_conv_cyc(a, a);
			fp48_exp(b, a, f);
			fp48_exp_cyc_sps(c, a, g, 3);
			TEST_ASSERT(fp48_cmp(b, c) == RLC_EQ, end);
			g[0] = 0;
			fp48_exp_cyc_sps(c, a, g, 0);
			TEST_ASSERT(fp48_cmp_dig(c, 1) == RLC_EQ, end);
			g[0] = 0;
			fp48_exp_cyc_sps(c, a, g, 1);
			TEST_ASSERT(fp48_cmp(c, a) == RLC_EQ, end);
			g[0] = -1;
			fp48_exp_cyc_sps(b, a, g, 1);
			fp48_inv(b, b);
			fp48_sqr_cyc(c, a);
			TEST_ASSERT(fp48_cmp(b, c) == RLC_EQ, end);
		} TEST_END;
...
static int cyclotomic54(void) {
	...
	TRY {
		...
		TEST_BEGIN("sparse cyclotomic exponentiation is correct") {
			...
			fp54_conv_cyc(a, a);
			fp54_exp(b, a, f);
			fp54_exp_cyc_sps(c, a, g, 3);
			TEST_ASSERT(fp54_cmp(b, c) == RLC_EQ, end);
			g[0] = 0;
			fp54_exp_cyc_sps(c, a, g, 0);
			TEST_ASSERT(fp54_cmp_dig(c, 1) == RLC_EQ, end);
			g[0] = 0;
			fp54_exp_cyc_sps(c, a, g, 1);
			TEST_ASSERT(fp54_cmp(c, a) == RLC_EQ, end);
			g[0] = -1;
			fp54_exp_cyc_sps(b, a, g, 1);
			fp54_inv(b, b);
			fp54_sqr_cyc(c, a);
			TEST_ASSERT(fp54_cmp(b, c) == RLC_EQ, end);
		} TEST_END;
