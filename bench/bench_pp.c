...
static void pairing8(void) {
	...
	#if PP_EXT == BASIC || !defined(STRIP)
		BENCH_BEGIN("pp_dbl_k8_projc_basic") {
			ep2_rand(p[0]);
			ep_rand(q[0]);
	...
	#if PP_EXT == LAZYR || !defined(STRIP)
		BENCH_BEGIN("pp_dbl_k8_projc_lazyr") {
			ep2_rand(p[0]);
			ep_rand(q[0]);
	...
	BENCH_BEGIN("pp_exp_k8") {
		fp8_rand(e);
		BENCH_ADD(pp_exp_k8(e, e));
	}
	BENCH_END;

	BENCH_BEGIN("pp_map_oate_k8") {
		ep2_rand(p[0]);
		ep_rand(q[0]);
...
static void pairing48(void) {
	...
	BENCH_BEGIN("pp_exp_k48") {
		fp48_rand(e);
		BENCH_ADD(pp_exp_k48(e, e));
	}
	BENCH_END;

	BENCH_BEGIN("pp_map_k48") {
		fp8_rand(qx);
		fp8_rand(qy);
		fp8_rand(qz);
		ep_rand(p);
...
static void pairing54(void) {
	...
	BENCH_BEGIN("pp_exp_k54") {
		fp54_rand(e);
		BENCH_ADD(pp_exp_k54(e, e));
	}
	BENCH_END;

	BENCH_BEGIN("pp_map_k54") {
		fp9_rand(qx);
		fp9_rand(qy);
		fp9_rand(qz);
		ep_rand(p);
